...
class EquirectangularNode {
public:
    explicit EquirectangularNode(const ros::NodeHandle& nh);
    ~EquirectangularNode();

private:
    // Callback functions
    void imageCallback(const sensor_msgs::ImageConstPtr& msg);
    // rcl_interfaces::msg::SetParametersResult parametersCallback(const
    ...
    cv::Mat createEquirectangular(const cv::Mat& front_img,
                                  const cv::Mat& back_img);

    // ROS1 communication

    ros::Subscriber dual_fisheye_sub_;
    ros::Publisher  equirect_pub_;
    ros::NodeHandle pnh_;
