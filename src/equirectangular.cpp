...
EquirectangularNode::EquirectangularNode(const ros::NodeHandle& nh)
    : pnh_(nh),
    ...
    ROS_INFO(
        "C++ equirectangular node - CPU processing only (GPU via Python node)");

    // Add parameter callback
    // auto params_callback_handle = add_on_set_parameters_callback(std::bind(
    // &EquirectangularNode::parametersCallback, this, std::placeholders::_1));
    ...
    dual_fisheye_sub_ = pnh_.subscribe(
        "/insta/image", 10, &EquirectangularNode::imageCallback, this);

    equirect_pub_ =
        pnh_.advertise<sensor_msgs::Image>("/equirectangular/image", 10);
}
...
void EquirectangularNode::loadParameters() {
    ...
    try {
        cx_offset_   = pnh_.param<double>("cx_offset", 0.0);
        cy_offset_   = pnh_.param<double>("cy_offset", 0.0);
        crop_size_   = pnh_.param<int>("crop_size", 960);
        out_width_   = pnh_.param<int>("out_width", 1920);
        out_height_  = pnh_.param<int>("out_height", 960);
        gpu_enabled_ = pnh_.param<bool>("gpu", true);

        std::vector<double> translation{0.0, 0.0, -0.105};
        std::vector<double> rotation_deg{-0.5, 0.0, 1.1};
        pnh_.getParam("translation", translation);
        pnh_.getParam("rotation", rotation_deg);
        tx_    = translation[0];
        ty_    = translation[1];
        tz_    = translation[2];
        ...
        ROS_INFO("  Crop size: %d", crop_size_);
        ROS_INFO("  Center offset: (%.1f, %.1f)", cx_offset_, cy_offset_);
        ROS_INFO("  Translation: [%.3f, %.3f, %.3f]", tx_, ty_, tz_);
        ROS_INFO("  Rotation (deg): [%.1f, %.1f, %.1f]", rotation_deg[0],
                 rotation_deg[1], rotation_deg[2]);
        ROS_INFO("  Output size: %dx%d", out_width_, out_height_);
        ROS_INFO("  GPU enabled: %s", gpu_enabled_ ? "true" : "false");
...
void EquirectangularNode::updateCameraParameters() {
    ...
    cv::Mat Rx = (cv::Mat_<double>(3, 3) << 1.0, 0.0, 0.0, 0.0, cos(roll_),
                  -sin(roll_), 0.0, sin(roll_), cos(roll_));

    cv::Mat Ry = (cv::Mat_<double>(3, 3) << cos(pitch_), 0.0, sin(pitch_), 0.0,
                  1.0, 0.0, -sin(pitch_), 0.0, cos(pitch_));

    cv::Mat Rz = (cv::Mat_<double>(3, 3) << cos(yaw_), -sin(yaw_), 0.0,
                  sin(yaw_), cos(yaw_), 0.0, 0.0, 0.0, 1.0);
    ...
    if (maps_initialized_) {
        maps_initialized_ = false;
        ROS_INFO("Parameters updated, remapping will occur on next image");
    }
}

void EquirectangularNode::initMapping(int img_height, int img_width) {
    ROS_INFO(
        "Initializing equirectangular projection: fusing two %dx%d "
        "fisheye images to %dx%d",
    ...
    // Note: x=0 corresponds to lon=-π, x=out_width-1 corresponds to
    // lon=π*(out_width-1)/out_width
    ...
    cv::exp(-latitude,
            cos_lat);  // Using exp(-x) as intermediate for cos calculation
    ...
    for (int y = 0; y < out_height_; ++y) {
        for (int x = 0; x < out_width_; ++x) {
            if (back_mask_.at<uchar>(y, x)) {
                cv::Vec3d point(X.at<float>(y, x), Y.at<float>(y, x),
                                Z.at<float>(y, x));

                // Transform point
                cv::Mat point_mat   = cv::Mat(point);
                cv::Mat transformed = back_to_front_rotation_ * point_mat +
    ...
    ROS_INFO("Mapping matrices initialization complete");
}

cv::Mat EquirectangularNode::createEquirectangular(const cv::Mat& front_img,
    ...
    if (!maps_initialized_) {
        ROS_ERROR("Mapping arrays not properly initialized");
        return cv::Mat::zeros(out_height_, out_width_, CV_8UC3);
    }

    cv::Mat front_result, back_result;
    cv::remap(front_img, front_result, front_map_x_, front_map_y_,
              cv::INTER_CUBIC, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));
    cv::remap(back_img, back_result, back_map_x_, back_map_y_, cv::INTER_CUBIC,
...
void EquirectangularNode::imageCallback(
    const sensor_msgs::ImageConstPtr& dual_fisheye_msg) {
    try {
        cv_bridge::CvImagePtr cv_ptr =
            cv_bridge::toCvCopy(dual_fisheye_msg, "rgb8");
        ...
        cv::Mat front_img_full =
            dual_fisheye_img(cv::Rect(midpoint, 0, midpoint, img_height));
        cv::Mat back_img_full =
            dual_fisheye_img(cv::Rect(0, 0, midpoint, img_height));

        // cv::rotate(front_img_full, front_img_full,
        ...
        if (orig_h != current_crop_size || orig_w != current_crop_size) {
            ...
            if (y_start >= 0 && x_start >= 0 &&
                ...
                front_img = front_img_full(cv::Rect(
                    x_start, y_start, current_crop_size, current_crop_size));
                back_img  = back_img_full(cv::Rect(
                    x_start, y_start, current_crop_size, current_crop_size));
            } else {
        ...
        auto process_time = (ros::Time::now() - start_time).toSec();
        ROS_DEBUG("Processing time: %.3f seconds", process_time);

    } catch (const cv_bridge::Exception& e) {
        ROS_ERROR("cv_bridge exception: %s", e.what());
    } catch (const std::exception& e) {
        ROS_ERROR("Error processing images: %s", e.what());
    }
}

// rcl_interfaces::msg::SetParametersResult
// EquirectangularNode::parametersCallback(
...
//             param.get_name() == "out_height" || param.get_name() == "gpu") {
//             update_needed = true;
//         }
//     }
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "equirectangular");
    ros::NodeHandle     nh("~");
    EquirectangularNode node(nh);
    ros::spin();
    ros::shutdown();
    return 0;
}